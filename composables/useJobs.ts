...
export function useJobs() {
  ...
  async function setStatus(id: number, status: Status) {
    await $fetch(`/api/groups/${id}/status`, { method: 'PATCH', body: { status } })
    const g = groups.value.find((x) => x.id === id)
    if (g) g.status = status
  }

  async function setNotes(id: number, notes: string) {
    await $fetch(`/api/groups/${id}/notes`, { method: 'PATCH', body: { notes } })
    const g = groups.value.find((x) => x.id === id)
    if (g) g.notes = notes
  }

  return { filters, groups, loading, refresh, setStatus, setNotes }
}
...
export function useScrape() {
  ...
  async function trigger(sources?: string[]) {
    running.value = true
    try {
      const r = await $fetch('/api/scrape', { method: 'POST', body: { sources } })
      lastResult.value = r
      return r
    } finally {
      running.value = false
...
export function useNewCount() {
  ...
  async function peek(): Promise<number> {
    const r = await $fetch<{ count: number }>('/api/new-since-visit', { query: { peek: '1' } })
