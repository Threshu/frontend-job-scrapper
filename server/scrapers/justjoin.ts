...
const FRONTEND_SKILL_RE =
	/\b(vue|react|angular|svelte|nuxt|next|frontend|front-end|front end|javascript|typescript|html|css)\b/i;
const FRONTEND_TITLE_RE =
	/\b(front[- ]?end|frontend|fullstack|full[- ]?stack|web|javascript|typescript|vue|react|angular|svelte)\b/i;
...
function mapSalary(offer: JjitListOffer): {
	...
	const et =
		offer.employmentTypes.find((e) => e.from != null || e.to != null) ??
		offer.employmentTypes[0];
	if (!et) return {};
	return {
...
function buildRawJob(detail: JjitDetailOffer): RawJob {
	...
	return {
		...
		remote:
			...
			detail.workplaceType === "partly_remote",
		salaryMin: sal.min,
		salaryMax: sal.max,
...
async function fetchDetail(
	slug: string,
	signal?: AbortSignal,
): Promise<JjitDetailOffer> {
	const res = await fetch(`${BASE}/offers/${slug}`, {
		headers: HEADERS,
		signal,
	});
